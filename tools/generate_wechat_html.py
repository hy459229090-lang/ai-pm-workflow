...
"""
公众号文章 HTML 生成器
将 Markdown 格式的文章转换为公众号后台可直接使用的 HTML
"""
...
def md_to_wechat_html(md_content: str) -> str:
    ...
    processed_lines = []
    for line in lines:
        if line.strip() and not line.strip().startswith('<') and not line.strip().endswith('</h1>') and not line.strip().endswith('</h2>') and not line.strip().endswith('</h3>'):
...
def main():
    # 读取 Markdown 文件
    md_file = Path('publish/wechat/05-agent-team-full.md')
    ...
    # 输出
    output_file = Path('tools/wechat-05-full.html')
