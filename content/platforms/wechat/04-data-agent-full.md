...
<strong>验证数据</strong>：毛球首页从时间线改成推荐 feed 后，上线前看转化漏斗和功能渗透率
</div>

<div style="background:#f8f9fa;border-left:4px solid #07c160;padding:16px 20px;margin:20px 0;border-radius:0 4px 4px 0;">
<strong>收集数据</strong>：周报月报的核心指标（DAU、留存、发布率），老板突然问的"宠物档案完善率多少"
</div>

<div style="background:#f8f9fa;border-left:4px solid #07c160;padding:16px 20px;margin:20px 0;border-radius:0 4px 4px 0;">
<strong>产品分析</strong>：用户行为路径（浏览帖子 → 点赞 → 关注）、功能使用情况、异常波动定位
</div>

<div style="background:#f8f9fa;border-left:4px solid #07c160;padding:16px 20px;margin:20px 0;border-radius:0 4px 4px 0;">
<strong>快速汇报</strong>：临时被叫去开会，5 分钟拉出毛球社区和商城的关键数据
</div>
...
上周五下午 4 点，老板突然在群里问："毛球最近新功能的渗透率怎么样？"

要是以前，我得：
1. 打开数据平台
2. 找毛球的表（`t_event_log`？`t_post`？哪张是新功能的？）
3. 写 SQL（还得回忆字段名）
4. 跑出来数据
5. 复制到 Excel 做图
...
<p style="margin:0 0 16px 0;font-weight:600;color:#1a1a1a;">用户问："毛球过去 7 天的 DAU 怎么样？"</p>
...
<p style="margin:8px 0;color:#555;"><span style="display:inline-block;width:24px;height:24px;background:#07c160;color:#fff;border-radius:50%;text-align:center;line-height:24px;font-size:12px;margin-right:8px;">2</span>查数据字典：毛球用户行为日志在 t_event_log，user_id 标识用户</p>
<p style="margin:8px 0;color:#555;"><span style="display:inline-block;width:24px;height:24px;background:#07c160;color:#fff;border-radius:50%;text-align:center;line-height:24px;font-size:12px;margin-right:8px;">3</span>应用业务规则：排除测试账号(user_id < 1000)、只算正式环境</p>
