...
| 字段 | 含义 | 说明 |
|------|------|------|
| user_id | 用户 ID | 从 1000 开始，<1000 是测试数据 |
| username | 用户名 | - |
| created_at | 注册时间 | - |
| last_login_time | 最后登录时间 | 判断活跃用这个 |
...
|------|------|------|
| event_id | 事件 ID | - |
| user_id | 用户 ID | 关联 t_user |
| event_type | 事件类型 | login/purchase/share 等预定义枚举 |
| event_time | 事件时间 | - |
| properties | 事件属性 | JSON 格式 |
